ARG from=condaforge/miniforge3:latest
FROM ${from}
...
# RUN conda create -y -p $CONDA_PREFIX "python==$CONDA_PYTHON_VERSION.*"  -c conda-forge
...
RUN mkdir zero_order_walking_ergocub
WORKDIR /zero_order_walking_ergocub/
COPY . . 
RUN mkdir results
