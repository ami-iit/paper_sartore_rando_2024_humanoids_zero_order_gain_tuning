...
#        sorted_fitness.reverse()
    ...
    def _crossover(self, parents, offspring_size):
        ...
        for i in range(offspring.shape[0]):
            crossover_idx = self.rnd_state.randint(0, high=len(self.bounds), size=1)[0]
            p1_idx = i % parents.shape[0]
            p2_idx = (i + 1) % parents.shape[0]
            offspring[i, :crossover_idx] = parents[p1_idx, :crossover_idx]
            offspring[i, crossover_idx:] = parents[p2_idx, crossover_idx:]
        return offspring
