...
no_torque = True# int(sys.argv[1]) == 0
num_workers = 100 # int(sys.argv[2])
num_reps = 10 #int(sys.argv[3]) if len(sys.argv) > 3 else 1
...
for i in range(num_reps):
    ...
    es = ng.optimizers.EvolutionStrategy(recombination_ratio=1.0, popsize=popsize, offsprings=popsize-1).set_name("EVS", register=False)

    optimizer = es(parametrization=x0, budget=T * popsize, num_workers=num_workers)
...
no_torque = False# int(sys.argv[1]) == 0
num_workers = 100 # int(sys.argv[2])
num_reps = 10 #int(sys.argv[3]) if len(sys.argv) > 3 else 1
...
for i in range(num_reps):
    ...
    es = ng.optimizers.EvolutionStrategy(recombination_ratio=1, popsize=popsize, offsprings=popsize-1).set_name("EVS", register=False)
